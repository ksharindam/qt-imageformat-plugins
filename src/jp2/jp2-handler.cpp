...
bool isJp2(QIODevice *device)
{
    ...
    if (memcmp(bytes.constData(), "\x00\x00\x00\x0c\x6a\x50\x20\x20\x0d\x0a\x87\x0a",12)==0)
        return true;
    return false;
}
...
QImage readImage(QIODevice *device)
{
    ...
    QStringList clrspc_str = {"Unspecified", "sRGB", "Gray", "YUV", "eYCC", "CMYK"};
    QImage image;
    int w, h, depth, channels, colorspace;

    OPJ_CODEC_FORMAT format = OPJ_CODEC_JP2;//can be OPJ_CODEC_J2K
